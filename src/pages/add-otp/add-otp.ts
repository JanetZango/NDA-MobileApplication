...
export class AddOtpPage {
  ...
  verifyOTP(otpForm: NgForm) {
    if (otpForm.value.otp === '') {
      ...
      this.authService.checkOTP(otpForm.value.otp).subscribe((_responseData: AuthResponseData) => {
        let date = new Date();
        ...
        let user = new User(
          _responseData.guid,
          _responseData.full_name,
          _responseData.email,
          _responseData.province_guid,
          _responseData.access_token,
